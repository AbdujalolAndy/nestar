...
export class MemberService {
    ...
    public async login(input: LoginInput): Promise<Member> {
        ...
        const isMatch = await this.authService.comparePassord(memberPassword, response.memberPassword);
        if (!isMatch) {
    ...
    public async memberStatsEditor(input: StatisticModifier): Promise<Member> {
        ...
        const result = await this.memberModel
            .findOneAndUpdate({ _id: _id, memberStatus: MemberStatus.ACTIVE }, { $inc: { [targetKey]: modifier } }, { new: true })
            .exec()
        return result
    }
}
